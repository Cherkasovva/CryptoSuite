...
namespace Crypto.DES
{
    internal class DESRoundFunction
    {
        ...
        public byte[] Transform(byte[] right32, byte[] roundKey48)
        {
            if (right32 == null || roundKey48 == null) throw new ArgumentNullException();
            ...
            for (int s = 0; s < 8; s++)
            {
                ...
                for (int b = 0; b < 4; b++)
                {
                    int bitVal = (sVal >> (3 - b)) & 1;
                    int dstIdx = outBitBase + b;
                    int dstByte = dstIdx / 8;
                    int dstBitInByte = dstIdx % 8;
                    sOut[dstByte] |= (byte)(bitVal << (7 - dstBitInByte));
                }
            }

            var pOut = BitPermutor.Permute(sOut, P, bitsIndexedLsbFirst: false, indexStartsAtOne: true);
            return pOut;
        }
    }
}